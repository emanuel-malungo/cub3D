...
void	perform_dda(t_cub3d *cub3d, t_ray *ray)
{
	...
	while (!hit)
	{
		...
		if (cub3d->m.map_content[ray->mapY][ray->mapX] == '1')
			hit = 1;
	}
}
...
void	draw_texture_line(t_cub3d *cub3d, int x, t_ray *ray, double wallX)
{
	...
	if (ray->side == 0 && ray->rayDirX > 0)
		texture = &cub3d->t.texture_west;
	else if (ray->side == 0 && ray->rayDirX < 0)
		texture = &cub3d->t.texture_east;
	else if (ray->side == 1 && ray->rayDirY > 0)
		texture = &cub3d->t.texture_north;
	else
		texture = &cub3d->t.texture_south;
...
int	ray_casting(t_cub3d *cub3d)
{
	...
	mlx_put_image_to_window(cub3d->mlx_ptr, cub3d->mlx_win_ptr, cub3d->i.img, 0,
		0);
	return (1);
}
